...
import { Room as RoomType } from '../../type'
...
export default function Room() {
  ...
  const refIds = useRef<{ id: string; userName: string } | undefined>()
  const {
    handleSetUsers,
    room,
    handleSetRoom,
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      await serviceGame.listen(SOCKET_MESSAGES.ROOM_DATA, (data) => {
        handleSetRoom(data as RoomType)
        handleSetUsers((data as RoomType).listUser ?? [])
      })
      ...
      serviceGame.listen(SOCKET_MESSAGES.RESTART_ROOM, (roomDTO) => {
        const roomDt = roomDTO as RoomType
    ...
    <>
      ...
      {room ? (
        <article className={styles.container}>
          <BackToRooms roomId={room.id} className={styles.buttonBack} />
          <header>
