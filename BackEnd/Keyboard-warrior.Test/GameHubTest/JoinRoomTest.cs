...
namespace Keyboard_warrior.Test.GameHubTest
{
    ...
    public class JoinRoomTest
    {
        ...
        public async Task JoinRoom_Success()
        {
            ...
            //act
            var joinRoom = await _connection.InvokeAsync<SocketResponseDTO<RoomDTO?>>(
                nameof(GameHub.JoinRoom),
                roomCreated.Id,
                user.UserName
            );
        ...
        public async Task JoinRoom_Denied_WhenIsPlaying()
        {
            ...
            var roomCreated = await _utils.CreateRoom(
                _connection,
                firstUser.UserName,
                roomTextType
            );
            ...
            await _connection.InvokeAsync<SocketResponseDTO<RoomDTO?>>(
                nameof(GameHub.JoinRoom),
                roomCreated.Id,
                firstUser.UserName
            );
            ...
            await _connection.SendAsync(nameof(GameHub.Ready), firstUser.UserName, roomCreated.Id);

            await _connection.SendAsync(nameof(GameHub.Ready), secondUser.UserName, roomCreated.Id);

            //act
            var responseJoin = await _connection.InvokeAsync<SocketResponseDTO<RoomDTO?>>(
                nameof(GameHub.JoinRoom),
                roomCreated.Id,
                thirdUser.UserName
            );
