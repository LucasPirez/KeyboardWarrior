...
namespace Keyboard_warrior.Test.GameHubTest
{
    ...
    public class CreateRoomTest
    {
        ...
        public async Task CreateRoom_Success()
        {
            ...
            _connection.On<RoomDTO>("CreateRoom", o =>
            {
                roomNameReceived = o.Name;
            });

            var user = await _utils.Login(_connection);
            var response = await _connection.InvokeAsync<SocketResponseDTO<RoomDTO?>>(_methodName, user.UserName, roomName,roomTextType);
